...
def evaluate_answer(user_answer, expected_keywords):
    # Pre-process user_answer: make it lowercase and remove unnecessary punctuation
    processed_answer = re.sub(r'[^\w\s]', '', user_answer.lower())
    
    matched_keywords = [kw for kw in expected_keywords if kw.lower() in processed_answer]
...
if st.session_state['question_asked'] and st.button("Record Speech"):
    with sr.Microphone() as source:
        ...
        try:
            ...
            st.text_area("Your Answer (as recognized by speech recognition):", st.session_state['user_answer'])
